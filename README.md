...
| 18 | **Telegram** | Bot token + chat ID → `/usr/local/bin/telegram-notify`. When configured, the full install **summary report is sent to your Telegram** as a document at the end of every run (quick / unattended / full). Optionally installs a **login alert** (`🚨 Login detected` — server/user/IP/time) via `pam_exec` hooks in `/etc/pam.d/sshd` **and** `/etc/pam.d/login`, firing on every SSH session (password, key, scp/sftp) **and local console login**. The script (`/usr/local/bin/telegram-login-alert.sh`) is also runnable by hand to test. |
...
/usr/local/bin/telegram-login-alert.sh # Login → Telegram (pam_exec: sshd + login, if enabled)
